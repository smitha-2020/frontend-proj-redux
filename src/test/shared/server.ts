...
const handler = [
    ...
    rest.put("https://api.escuelajs.co/api/v1/categories/:id", async (req, res, ctx) => {
        const { id } = req.params;
        const foundCategory = categoryList.find((reqProduct:ICategory) => reqProduct.id === Number(id))
    ...
    rest.put("https://api.escuelajs.co/api/v1/users/:id", async (req, res, ctx) => {
        ...
        const foundUser: IAuthenticUser[] = users.filter((user) => { return user.id === Number(idUrl) })
